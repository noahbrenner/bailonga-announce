...
class ViewModel {
  ...
  public facebookGroupCheckboxState = objectFromEntries(
    ALL_FACEBOOK_GROUPS.map((groupName) => [groupName, ko.observable(false)]),
  );
  ...
  private serializedState = ko.pureComputed((): IState => {
    ...
    return {
      ...
      musicType,
      customMusicType:
        musicType === this.OTHER_MUSIC_TYPE ? this.customMusicType() : "",
  ...
  private templateLocals = ko.pureComputed((): ITemplateLocals => {
    ...
    return {
      ...
      musicType:
        musicType === this.OTHER_MUSIC_TYPE ? customMusicType : musicType,
  ...
  private loadState(): IState {
    ...
    try {
      const storageItem = localStorage.getItem(ViewModel.LOCAL_STORAGE_KEY);
      if (!storageItem) {
        return fallback;
      }
      ...
      return {
        ...
        photoCredit: getString(stored.photoCredit, fallback.photoCredit),
  ...
  private getDefaultVenue(eventDate: Date): Venue {
    return nthWeekdayOfMonth(eventDate) === 2 ? "Colonial Room" : "Ballroom";
  }
